...
import com.teamteam.knowing.data.model.network.response.CategoryCorrectResponse
...
class CategoryCorrectActivityViewModel(application: Application) : AndroidViewModel(application) {
    private val myContext = application.applicationContext

    //api 성공 여부 저장하는 라이브 데이터
    ...
    /*
    카테고리 수정하는 api 호출 메소드
     */
    fun tryPostCategoryCorrect(userUid:String){
        val category = CategoryCorrectRequest(getStudentCategory(),getJobCategory(),getFoundationCategory(),getResidentCategory(),getLifeCategory(),getCovidCategory())
        val signUpInterface = ApplicationClass.sRetrofit.create(SignUpInterface::class.java)
        signUpInterface.postCategoryCorrect(userUid, category)
            .enqueue(object : Callback<CategoryCorrectResponse> {
                override fun onResponse(call: Call<CategoryCorrectResponse>, response: Response<CategoryCorrectResponse>) {
                    if (response.isSuccessful){
                        val result = response.body()  as CategoryCorrectResponse
                        if (result.categoryCorrectResult.status=="유저정보수정완료"){
                            _isSuccessApi.value=true
                        }
                    }else{
                        Log.e("ERROR","카테고리 수정 실패")
                        _isSuccessApi.value=true
                    }
                }

                override fun onFailure(call: Call<CategoryCorrectResponse>, t: Throwable) {
                    Log.e("ERROR","카테고리 수정 통신 실패")
                    _isSuccessApi.value=true
                }
            })
    }
