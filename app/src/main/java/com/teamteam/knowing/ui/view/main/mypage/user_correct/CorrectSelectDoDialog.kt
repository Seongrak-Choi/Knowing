...
class CorrectSelectDoDialog() : DialogFragment() {
    ...
    override fun onCreateView(
        ...
        moreInformationCorrectActivityViewModel.doList.observe(this, Observer {
            ...
            adapter.setOnItemClickListener(object : SelectDoDialogRCAdapter.OnItemClickListener {
                override fun onItemClick(value: String) {
                    moreInformationCorrectActivityViewModel.currentTxDo.value =
                        value  //리사이클러뷰에서 선택되어진 데이터를 뷰모델의 currentTxSi에 저장
                    moreInformationCorrectActivityViewModel.isSelectDo.value =
                        true //리사이클러뷰에서 시/도가 선택되어지면 선택이 되었다고 뷰모델의 isSelectDo에 true저장

                    //시/도/군을 선택한 상황에서 시/도를 변경하는 경우 시/군/구가 리셋 되도록 설정
                    moreInformationCorrectActivityViewModel.currentTxSi.value="시/군/구 선택"
                    moreInformationCorrectActivityViewModel.isSelectSi.value=false
